...
class PalmDetection(object):
    def __init__(
        ...
        iou_threshold: Optional[float] = 0.60,
    ...
    def __postprocess(
        ...
        #    # Filter boxes based on both score threshold and IoU threshold
        # for i in range(len(boxes)):
        #     if keep[i]:  # If the box passes the score threshold
        #         for j in range(i + 1, len(boxes)):
        #             if keep[j]:  # If the other box also passes the score threshold
        #                 iou = calculate_iou(boxes[i], boxes[j])
        #                 if iou > self.iou_threshold:
        #                     keep[j] = False  # Remove the box if IoU is above threshold
